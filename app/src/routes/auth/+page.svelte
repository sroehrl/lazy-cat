...
<form method="post">
    <input type="hidden" name="redirect" value={data.search.to || "/member"} />
    <div
        class="w-4/5 md:w-1/3 mx-auto border rounded-lg shadow dark:text-white"
    >
        <div
        ...
        >
            ...
            {#if form?.error}
                <Alert color="red">Wrong credentials</Alert>
            {/if}
            <div class="flex justify-between">
                <div class="text-sm">
                    <a href="/auth/forgot-password" class="text-red-500"
            ...
            <p class="text-sm">
                Not yet registered? <a
                    href="/auth/register"
                    class="text-primary-600">Become a member</a
                >
            </p>
        </div>
    </div>
