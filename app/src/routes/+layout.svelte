<script lang="ts">
  ...
  import {
      Button,
      DarkMode,
      Dropdown, DropdownDivider, DropdownItem,
      Footer, Indicator,
      Navbar,
      NavBrand,
      NavHamburger,
  ...
  import {
      ...
      FacebookSolid,
      GithubSolid,
      PhoneOutline, ShoppingCartSolid, UserCircleSolid
...
<Toast divClass="rounded-lg flex items-center w-full max-w-xs p-4 text-gray-100 {$toastStore.color} shadow dark:text-gray-200 gap-3 fixed top-1 right-1 z-10" position="none" open={$toastStore.open}>{$toastStore.message}</Toast>
<Navbar class="z-10 text-white bg-secondary-600 dark:bg-neutral-900 bg-opacity-80 md:py-10" let:hidden let:toggle>
    ...
    <NavBrand>
      ...
      <a href="{base}/">
          <img src="/logo.png" class="w-16 md:w-24 " alt="Lazy-Cat-Lounge-Logo">
      </a>
    </span>
    </NavBrand>
    <NavUl {hidden} activeUrl={$page.url.pathname}>
        <NavLi class="cursor-pointer text-2xl">About <ChevronDownOutline class="inline w-3"/></NavLi>
        ...
        <NavUl activeUrl={$page.url.pathname}>
            <NavLi class="cursor-pointer relative" href="{base}/checkout">
                ...
                {#if $order?.items?.length > 0}
                    <Indicator color="dark" size="lg" border placement="top-right">{$order?.items?.length}</Indicator>
                {/if}
            </NavLi>
            ...
            <Dropdown class="w-44 z-20">
                ...
                {:else}
                    ...
                    <DropdownDivider />
                    <DropdownItem><form method="post" action="?/signOut"><button type="submit">Sign out</button></form></DropdownItem>
                {/if}
...
<Footer class="bg-black text-white dark:bg-neutral-900">
    <div class="py-5 px-4 grid gap-3 md:grid-cols-3 items-center">
        <div class="flex gap-3 place-self-center">
            <Button pill class="!p-2" target="_blank" rel="noreferrer noopener" href="https://www.facebook.com/lazycatloungeandcafe/">
        ...
        <div class="md:flex gap-3 place-self-center">
            <p><a class="text-primary-600 flex gap-1" href="tel:+15854254202"><PhoneOutline class="w-3"/> 585.425.4202</a></p>
            <p><a class="text-primary-600 flex gap-1" href="mailto:lazycatloungeandcafe@gmail.com"><EnvelopeOutline class="w-3"/> lazycatloungeandcafe@gmail.com</a></p>
        </div>
        <div class="place-self-center">© {dayjs().format('YYYY')} Lazy Cat Lounge & Cafe. All Rights Reserved.</div>
    </div>
</Footer>
